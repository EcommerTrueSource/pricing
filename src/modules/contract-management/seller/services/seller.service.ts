...
export class SellerService {
  ...
  async updateFromBrasilApi(id: string, retryCount = 0): Promise<any> {
    try {
      this.logger.debug(`Tentando atualizar seller ${id} (tentativa ${retryCount + 1}/${this.MAX_RETRIES})`);
      ...
      this.logger.debug(`Dados antigos: razaoSocial=${seller.razaoSocial}, endereco=${seller.endereco}`);
      this.logger.debug(`Dados novos: razaoSocial=${updatedSeller.razao_social}, endereco=${updatedSeller.endereco}`);
      return updatedSeller;
    } catch (error) {
      this.logger.error(`Erro ao atualizar seller ${id}: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
      ...
      if (this.isTemporaryError(error) && retryCount < this.MAX_RETRIES) {
        this.logger.warn(`Aguardando ${this.RETRY_DELAY/1000} segundos antes de tentar novamente...`);
  ...
  async updateAllSellersDataFromBrasilApi(): Promise<{
    ...
    for (let i = 0; i < sellers.length; i += BATCH_SIZE) {
      ...
      this.logger.log(`Processando lote ${Math.floor(i / BATCH_SIZE) + 1} de ${Math.ceil(sellers.length / BATCH_SIZE)}`);

      for (const seller of batch) {
        try {
          await this.updateFromBrasilApi(seller.id);
          result.success++;
          this.logger.log(`Seller ${seller.id} (CNPJ: ${seller.cnpj}) atualizado com sucesso`);
          ...
          this.logger.error(`Erro ao atualizar seller ${seller.id} (CNPJ: ${seller.cnpj}): ${errorMessage}`);
        }
      }

      // Aguarda 5 segundos entre os lotes
      if (i + BATCH_SIZE < sellers.length) {
        this.logger.log(`Aguardando ${BATCH_DELAY / 1000} segundos antes do próximo lote...`);
  ...
  async updateRemainingSellersFromBrasilApi(): Promise<{
    ...
    success: number;
    failed: number;
    errors: Array<{ cnpj: string; error: string; type: 'invalid_cnpj' | 'not_found' | 'validation_error' | 'other' }>;
  }> {
    ...
    if (sellers.length > 0) {
      ...
      sellers.slice(0, 3).forEach(seller => {
        this.logger.log(`- CNPJ: ${seller.cnpj}, Razão Social: ${seller.razao_social}, Endereço: ${seller.endereco}`);
      });
    }
    ...
    for (let i = 0; i < sellers.length; i += BATCH_SIZE) {
      const batch = sellers.slice(i, i + BATCH_SIZE);
      this.logger.log(`Processando lote ${Math.floor(i / BATCH_SIZE) + 1} de ${Math.ceil(sellers.length / BATCH_SIZE)}`);
      this.logger.log(`Progresso: ${i + 1} a ${Math.min(i + BATCH_SIZE, sellers.length)} de ${sellers.length}`);
      ...
      for (const seller of batch) {
        try {
          ...
          result.failed++;
          const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido';
          let errorType: 'invalid_cnpj' | 'not_found' | 'validation_error' | 'other' = 'other';
